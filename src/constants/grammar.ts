...
export const GRAMMAR: Record<string, SyntaxRule[]> = {
	javascript: [
		...
		{ pattern: '["\'`][\\s\\S]*?["\'`]', color: 'text-green-600' },
		{
			pattern:
	...
	python: [
		...
		{ pattern: '\\b[a-zA-Z_][0-9a-zA-Z_]*(?=\\s*\\()', color: 'text-blue-600' },
		{ pattern: '\\b\\d+(?:\\.\\d+)?\\b', color: 'text-orange-500' },
	],
	markdown: [
		{ pattern: '^#+\\s+.*', color: 'text-blue-600 font-bold' },
		{ pattern: '\\*\\*.*?\\*\\*|__.*?__', color: 'font-bold text-gray-900' },
		{ pattern: '\\*.*?\\*|_.*?_', color: 'italic text-gray-700' },
		{ pattern: '`[^`]+`', color: 'text-green-700 bg-green-50/50 px-1 rounded' },
		{
			pattern: '^\\s*(?:[-*+]|\\d+\\.)\\s+',
