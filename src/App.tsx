...
const LiquidGlassStyles: React.FC = () => (
  <style>{`
    ...
    .liquid-border { position: relative; background: rgba(10, 10, 10, 0.45); z-index: 1; backdrop-filter: blur(10px); }
    ...
    /* CARD HOVER -> TRIGGERS BUTTON LIQUID BORDER */
    .group-hover-liquid { position: relative; z-index: 1; transition: background-color 0.3s; }
    .group-hover-liquid::before {
      content: ""; position: absolute; inset: -1.5px; padding: 1.75px; border-radius: inherit;
      background: linear-gradient(115deg, transparent 0%, transparent 20%, #ff8a00 28%, #e52e71 30%, #9c27b0 32%, transparent 40%, transparent 70%, #ff8a00 78%, #e52e71 80%, #9c27b0 82%, transparent 90%, transparent 100%);
      background-size: 200% auto; 
      -webkit-mask: linear-gradient(#fff 0 0) content-box, linear-gradient(#fff 0 0); -webkit-mask-composite: xor; mask-composite: exclude; z-index: -1;
      opacity: 0; transition: opacity 0.4s ease-in-out;
    }
    .group:hover .group-hover-liquid::before {
      opacity: 1; animation: shimmer 2s linear infinite;
    }
    .group:hover .group-hover-liquid {
      ...
      >
        {Icon && variant === 'liquid' ? (
          <div className='w-8 h-8 rounded-full bg-white/10 flex items-center justify-center backdrop-blur-md'>
            <Icon className='w-4 h-4 text-white group-hover:text-cyan-200 transition-colors' />
          </div>
        ) : (
          Icon && (
            <Icon
              className={cn(
                variant === 'liquid-icon'
                  ? 'w-5 h-5 text-gray-300 group-hover:text-gray-200 transition-colors'
                  : variant === 'icon'
        ...
        {children && (
          <span
            className={
              variant === 'liquid' ? 'text-white font-medium text-sm' : ''
            }
          >
            {children}
          </span>
...
const ProjCard: React.FC<CardProps<ProjectData>> = ({
  ...
  >
    ...
    <div className='relative h-56 overflow-hidden'>
      ...
      <div className='absolute top-4 left-4 glass-panel px-4 py-1.5 rounded-full flex items-center space-x-2'>
    ...
    <div className='p-6 flex flex-col justify-between flex-1 relative z-20'>
      ...
      <div className='flex gap-3 mt-auto pt-4 border-t border-black/5'>
        {/* BUTTON HOVER LIQUID EFFECT APPLIED HERE */}
        <GlassBtn
          variant='outline'
          icon={Github}
          className='flex-1 py-2 text-xs group-hover-liquid'
...
const CertCard: React.FC<CardProps<CertData>> = ({
  ...
  <motion.article
    ...
    transition={{ duration: 0.6, delay: 0.1 + index * 0.1 }}
    whileHover={{ y: -8 }}
    className='group relative glass-panel glass-panel-hover rounded-[2rem] p-6 flex flex-col w-full transition-all duration-500'
  >
    <div className='absolute inset-0 bg-gradient-to-tr from-white/40 via-white/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none z-10' />
...
const Hero: React.FC = () => {
  ...
  return (
    <section
      id='home'
      className='min-h-screen flex items-center px-4 sm:px-6 lg:px-20 pt-28 lg:pt-36 pb-16 max-w-7xl mx-auto'
    >
      <div className='grid lg:grid-cols-2 gap-12 lg:gap-16 items-center w-full'>
          ...
          <div className='h-8 sm:h-10 flex items-center justify-center lg:justify-start overflow-hidden'>
            <AnimatePresence mode='wait'>
              <motion.h2
                ...
                className='text-lg sm:text-xl md:text-2xl text-gray-600 font-light tracking-wide'
              >
                {ROLES[roleIdx]}
              </motion.h2>
            </AnimatePresence>
        ...
        <motion.div
          {...fadeInRight}
          className='w-full max-w-lg mx-auto glass-panel rounded-[2.5rem] p-2 relative animate-[float_4s_ease-in-out_infinite] liquid-border-light'
        >
          <div className='glass-inset rounded-[2rem] p-5 sm:p-6 backdrop-blur-xl border border-white/20'>
            <div className='flex justify-between items-center mb-5 sm:mb-6'>
              <div className='flex gap-2 items-center'>
                <div className='w-3 h-3 rounded-full bg-red-400 shadow-[0_0_8px_#f87171]' />
              ...
              <button
                ...
                className='glass-panel px-3 py-1.5 rounded-full hover:bg-white/80 transition-colors cursor-pointer'
              >
                {gen ? (
            ...
            <div className='min-h-[140px] sm:min-h-[160px] font-mono text-xs sm:text-sm relative overflow-hidden'>
              <motion.div
                key={tplIdx}
...
const About: React.FC = () => {
  ...
  return (
    <section
      id='about'
      ref={ref}
      className='py-16 sm:py-20 px-4 sm:px-6 max-w-7xl mx-auto overflow-hidden relative z-10'
    >
      ...
      <div className='grid lg:grid-cols-2 gap-8 sm:gap-10 items-center'>
        <motion.div
          ...
          className='glass-panel rounded-[2.5rem] p-2 w-full'
        >
          <div className='glass-inset rounded-[2rem] p-6 sm:p-8 backdrop-blur-xl overflow-hidden'>
            <div className='flex gap-2 mb-6 pb-4 border-b border-black/5'>
              <Terminal className='w-4 h-4 text-gray-500' />
              <span className='text-gray-500 text-[10px] sm:text-xs uppercase tracking-widest'>
                sys_profile.md
              </span>
            </div>
            <div className='space-y-4 text-xs sm:text-sm text-gray-700 leading-relaxed overflow-hidden w-full font-mono'>
              {ABOUT_MD.map((l, i) => (
                <motion.div
                  key={i}
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5 w-full'>
          {STATS.map((s, i) => (
            <motion.div
              ...
              whileHover={{ y: -5 }}
              className='glass-panel glass-panel-hover p-6 sm:p-8 rounded-[2rem] flex flex-col items-center text-center group transition-all duration-300'
            >
...
const SectionGrid = <T extends any>({
  ...
  return (
    <section
      id={id}
      ref={ref}
      className='py-16 sm:py-20 px-4 sm:px-6 max-w-7xl mx-auto relative z-10'
    >
      <div className='absolute top-0 left-1/4 right-1/4 h-px bg-gradient-to-r from-transparent via-gray-300 to-transparent' />
...
const Contact: React.FC = () => {
  ...
  return (
    <section
      id='contact'
      ref={ref}
      className='py-16 sm:py-24 px-4 sm:px-6 max-w-6xl mx-auto overflow-hidden relative z-10'
    >
      <div className='absolute top-0 left-1/4 right-1/4 h-px bg-gradient-to-r from-transparent via-gray-300 to-transparent' />
      <SectionHeader
        title="Let's Connect"
      ...
      <div className='grid lg:grid-cols-12 gap-8 sm:gap-12'>
        <motion.div
          {...fadeInLeft}
          animate={inView ? fadeInLeft.animate : {}}
          className='lg:col-span-5 glass-panel rounded-[2.5rem] p-8 h-full flex flex-col justify-center'
        >
          ...
          <div className='flex flex-col gap-5'>
            {CONTACT_INFO.map((i, idx) => (
              <motion.a
                ...
                className='flex gap-5 group items-center p-2 -m-2 rounded-2xl hover:bg-white/40 transition-colors'
              >
                <div className='w-12 h-12 glass-inset rounded-full flex justify-center items-center group-hover:scale-110 transition-transform'>
                  <i.icon className='w-5 h-5 text-gray-500 group-hover:text-gray-900 transition-colors' />
                </div>
                <div>
                  <p className='text-[10px] sm:text-xs uppercase tracking-widest text-gray-500 font-semibold mb-1'>
                    {i.label}
                  </p>
        ...
        <motion.div
          ...
          className='lg:col-span-7 glass-panel rounded-[2.5rem] p-8'
        >
          <h3 className='text-xl font-semibold text-gray-900 mb-8 tracking-wide'>
            Send a Message
          </h3>
...
const ArchivePage = <T extends any>({
  ...
  return (
    <div className='min-h-screen pt-28 lg:pt-36 px-4 sm:px-6 text-center pb-16 sm:pb-20 max-w-7xl mx-auto relative z-10'>
...
const Navbar: React.FC<{ nav: (route: string) => void }> = ({ nav }) => {
    ...
    if (h.startsWith('#')) {
      ...
      setTimeout(() => {
        ...
        if (el) {
          ...
          const offset = isDesktop ? 100 : 80; // adjusted for top fixed navbar
  ...
  return (
    <nav className='fixed top-4 left-0 right-0 z-[100] flex justify-center px-4'>
      <div className='glass-panel bg-white/10 backdrop-blur-3xl border border-white/40 shadow-2xl shadow-blue-900/5 rounded-full flex items-center justify-between p-1.5 w-full max-w-[20rem] max-w-[20rem] sm:max-w-md md:max-w-2xl lg:max-w-4xl'>
        <div
