...
export const Navbar: React.FC<{ nav: (route: string) => void }> = ({ nav }) => {
	...
	return (
		<nav className='fixed top-4 left-0 right-0 z-[100] flex justify-center px-4'>
			<div className='relative rounded-full p-1.5 w-full max-w-[20rem] sm:max-w-md md:max-w-2xl lg:max-w-5xl'>
				<div
					className={cn(
						'absolute inset-0 rounded-full transition-opacity duration-500 pointer-events-none',
						isScrolled ? 'liquid-glass-strong-nav opacity-100' : 'opacity-0'
					)}
				/>
			...
			<div className='sm:ml-4 px-3 flex items-center justify-center relative z-10'>
				<ThemeToggle />
			</div>
		</nav>
	);
